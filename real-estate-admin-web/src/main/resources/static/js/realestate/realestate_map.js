...
let loadCadastralMap = function(searchAddress) {
    ...
    let mapContainer = document.getElementById('cadastralMap'); // 지도를 표시할 div
    let mapOption = {
        center: new kakao.maps.LatLng(37.566826, 126.9786567), // 지도의 중심좌표
        level: 2 // 지도의 확대 레벨
    };
    ...
    let cadastralMap = new kakao.maps.Map(mapContainer, mapOption);
    let geocoder = new kakao.maps.services.Geocoder();
    ...
    geocoder.addressSearch(searchAddress, function(result, status) {
        ...
        // 지도의 중심을 결과값으로 받은 위치로 이동시킵니다
        cadastralMap.setCenter(coords);
        cadastralMap.addOverlayMapTypeId(mapTypes.useDistrict);
    });
}
